...
struct PAndLPostingsExecution: View {
    ...
    var body: some View {
        VStack {
            ...
            Divider()
            Print_Preview(accounts: $accounts, postings: $postings, pAndLLines: $pAndLLines, fechaInforme1desde: $fechaInforme1desde, fechaInforme1hasta: $fechaInforme1hasta, fechaInforme2desde: $fechaInforme2desde, fechaInforme2hasta: $fechaInforme2hasta, altura: $altura)
        }
    }
    ...
    private func onPrint() {
        ...
        let rootView = Print_Preview(accounts: $accounts, postings: $postings, pAndLLines: $pAndLLines, fechaInforme1desde: $fechaInforme1desde,fechaInforme1hasta: $fechaInforme1hasta, fechaInforme2desde: $fechaInforme2desde, fechaInforme2hasta: $fechaInforme2hasta, altura:$altura )
    ...
    struct Print_Preview: View {
        ...
        @State private var isPresentingPostingPAndL1Execution = false
        @State private var isPresentingPostingPAndL2Execution = false
        ...
        func importeDosCuenta ( postings:[Posting], cuenta:[PAndLLine.AccountsInLine]) -> Double  {
            ...
            let importe1 = postings.filter({($0.date >= fechaInforme2desde)&&($0.date <= fechaInforme2hasta) && (accountArray.contains( where: $0.firstAccount.elementsEqual))}).map{$0.debitAmount}.reduce(0, +)
        
            let importe2 = postings.filter({($0.date >= fechaInforme2desde)&&($0.date <= fechaInforme2hasta) && (accountArray.contains( where: $0.firstAccount.elementsEqual))}).map{$0.creditAmount}.reduce(0, +)
        
            let importe3 = postings.filter({($0.date >= fechaInforme2desde)&&($0.date <= fechaInforme2hasta) && (accountArray.contains( where: $0.secondAccount.elementsEqual))}).map{$0.debitAmount}.reduce(0, +)
        
            let importe4 = postings.filter({($0.date >= fechaInforme2desde)&&($0.date <= fechaInforme2hasta) && (accountArray.contains( where: $0.secondAccount.elementsEqual))}).map{$0.creditAmount}.reduce(0, +)
        ...
        var body: some View {
            ...
            Form {
                ...
                HStack{
                    Text (" Periodo 1 del \(fechaInforme1desde, style: .date) hasta \(fechaInforme1hasta, style: .date)")
                    Text (" Periodo 2 del \(fechaInforme2desde, style: .date) hasta \(fechaInforme2hasta, style: .date)")
                }
            Text("")
                .onAppear() { altura += 24 }
                    ...
                    HStack {
                        ...
                        Text (String(format: "%.2f", a ))
                            .onTapGesture {                                self.selectedLine = linea
                                isPresentingPostingPAndL1Execution = true}
                        Text (String(format: "%.2f", b ))
                            .onTapGesture {                                self.selectedLine = linea
                                isPresentingPostingPAndL2Execution = true}
            ...
            .sheet (isPresented: $isPresentingPostingPAndL1Execution) {
                NavigationView {
                    ...
                    PostingsPAndLPostingsExecution(accounts: $accounts, postings: $postings, pAndLLines: $pAndLLines, selectedLine: $selectedLine,fechaDesde: $fechaInforme1desde, fechaHasta: $fechaInforme1hasta)
                        .toolbar {
                            ToolbarItem(placement: .cancellationAction) {
                                Button("Dismiss") {
                                    isPresentingPostingPAndL1Execution = false
                                }
                            }
                        }
            ...
            .sheet (isPresented: $isPresentingPostingPAndL2Execution) {
                NavigationView {
                    
                    PostingsPAndLPostingsExecution(accounts: $accounts, postings: $postings, pAndLLines: $pAndLLines, selectedLine: $selectedLine,fechaDesde: $fechaInforme2desde, fechaHasta: $fechaInforme2hasta)
                        .toolbar {
                            ToolbarItem(placement: .cancellationAction) {
                                Button("Dismiss") {
                                    isPresentingPostingPAndL2Execution = false
                                }
                            }
                        }
